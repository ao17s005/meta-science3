...
```{r}
data_new <- data_amp_raw |> 
  filter(trialcode != "instructions", 
         trialcode != "prime_positive_practice",
         trialcode != "prime_negative_practice") |> 
  mutate(problematic = as.numeric(case_when(latency < 100 ~ "1", 
                                 .default = "0")),
         problematic_n = sum(problematic))
data_new <- data_amp_raw |> 
  group_by(problematic) |> 
  summarize(problematic_n = n())
