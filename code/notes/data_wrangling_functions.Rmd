...
Useful arguments: skip = n, number of rows I want to skip
...
I can use colnames to check on the names of the dataset
...
I can use this function to clean the names of all the variables in the dataset. 
It should be included in the normal data flow.
```{r}
...
changes, which rows are present without changing their order (vs. arrange () which
changes the order of the rows, but not which rows are in or out)
...
## slice: to cut out rows or retaining rows

We might ue it just after reading in files if there are junk rows at the header.

```{r}

# dplyr::slice()
mtcars # this should have 32 rows
slice(mtcars, 2, n()) # built in data, this should have 31 rows

```

## select

selects the columns, I want to keep in my data 1) new name = old name , 2) name1: name5 
= selects all in between, 3) !name1:name5 = all columns except them.

helper functions: starts_with("a"), ends_with("z"), contains("m"),
num_range(x, 1:3) = matches x1, x2 and x3

What is very handy, is, you can directly rename in select.
...
for creating new columns with the existing data
with the .before argument I can add the new column to the left hand side
